...
app.get('/api/health', async (req, res) => {
  try {
    ...
    // Updated to match native MySQL array destructuring format
...
if (process.env.NODE_ENV === 'production') {
  // Serve static assets from your build directory (e.g., Vite/React dist folder)
  app.use(express.static(path.join(__dirname, '../dist')));
  
  // Standard Express wildcard catch-all route to handle React Router client-side routes
  app.get('*', (req, res) => {
